...
// --- State Variables ---
const SETTINGS_VERSION = '3.0'; // Updated version for settings changes
let currentTheme = {};
...
// --- Functions ---
function applyRubrication(text, langKey) {
...
function loadSettings() {
    ...
    if (savedVersion !== SETTINGS_VERSION) {
        ...
        if (savedLanguageOrder) {
            ...
            // Basic validation to ensure the loaded order isn't corrupted
...
function formatPrayerText(text, langKey, query) {
    let processedText = text;
    const lang = langKey.split('_')[0];
    ...
    processedText = applyRubrication(processedText, langKey);
    ...
    prayers.forEach(prayer => {
        ...
        languageOrder.forEach(langKey => {
            if (displayedLanguages[langKey] && prayer[langKey] && prayer[langKey].trim()) {
                ...
                langText.innerHTML = formatPrayerText(prayer[langKey], langKey, searchQuery);
